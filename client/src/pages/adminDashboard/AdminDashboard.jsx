...
import {
    ...
    FiX, FiActivity, FiClipboard, FiFileText, FiChevronDown, FiTag, FiClock as FiDelivery
} from "react-icons/fi";
...
const AdminDashboard = () => {
    ...
    const [viewGigTarget, setViewGigTarget] = useState(null);  // view-only, no edit
    ...
    const fetchGigs = async () => {
        try {
            ...
            setGigsError("Failed to fetch gigs.");
    ...
    // Role badge — user is either student or faculty
    const creatorRoleBadge = (u) => {
        if (!u) return null;
        if (u.isAdmin) return <span className="badge badge-admin"><FiZap className="icon-sm" /> Admin</span>;
        if (u.role === "faculty") return <span className="badge badge-faculty">🎓 Faculty</span>;
        return <span className="badge badge-buyer"><FaUserGraduate className="icon-sm" /> Student</span>;
    };

    // ── Overview chart data ────────────────────────────────────────
    const userBreakdown = [
        { name: "Faculty", value: stats.totalSellers || 0, color: "#f59e0b" },
        { name: "Students", value: stats.totalBuyers || 0, color: "#7c3aed" },
    ];
    ...
    const orderBreakdown = [
        { name: "Completed", value: orderStats.completedOrders, color: "#46a91f" },
        { name: "In Progress", value: orderStats.pendingOrders, color: "#f59e0b" },
    ];
    const topGigs = [...gigs]
    ...
    const topRated = [...gigs]
        ...
        .map((g) => ({
            name: g.title?.length > 20 ? g.title.slice(0, 20) + "…" : g.title,
            rating: parseFloat((g.totalStars / g.starNumber).toFixed(1)),
        }));
        ...
        <div className="admin-layout">
            ...
            <aside className={`admin-sidebar ${sidebarOpen ? "open" : "collapsed"}`}>
                <div className="sidebar-header">
                    <div className="sidebar-logo">
                        {sidebarOpen ? <span className="logo-text">Campus<span className="logo-accent">Connect</span></span> : <span className="logo-icon">CC</span>}
                    </div>
                ...
                <nav className="sidebar-nav">
                    <button className={`nav-item ${activeNav === "overview" ? "active" : ""}`} onClick={() => setActiveNav("overview")}><span className="nav-icon"><FiPieChart /></span>{sidebarOpen && <span className="nav-label">Overview</span>}</button>
                    <button className={`nav-item ${activeNav === "users" ? "active" : ""}`} onClick={() => setActiveNav("users")}><span className="nav-icon"><FiUsers /></span>{sidebarOpen && <span className="nav-label">Users</span>}</button>
                    <button className={`nav-item ${activeNav === "gigs" ? "active" : ""}`} onClick={() => setActiveNav("gigs")}><span className="nav-icon"><FiFileText /></span>{sidebarOpen && <span className="nav-label">Gigs</span>}</button>
                    <button className={`nav-item ${activeNav === "orders" ? "active" : ""}`} onClick={() => setActiveNav("orders")}><span className="nav-icon"><FiPackage /></span>{sidebarOpen && <span className="nav-label">Orders</span>}</button>
                    <Link to="/" className="nav-item nav-link"><span className="nav-icon"><FiGlobe /></span>{sidebarOpen && <span className="nav-label">View Site</span>}</Link>
                </nav>
                <div className="sidebar-bottom">
                    <div className="admin-profile">
                        <img src={currentUser?.img || "/img/noavatar.jpg"} alt="admin" className="admin-avatar" />
                        {sidebarOpen && <div className="admin-info"><p className="admin-name">{currentUser?.username}</p><p className="admin-role">Administrator</p></div>}
                    </div>
                    <button className="logout-btn" onClick={handleLogout}><span className="nav-icon"><FiLogOut /></span>{sidebarOpen && <span>Logout</span>}</button>
                </div>
            </aside>
            ...
            <main className="admin-main">
                <header className="admin-header">
                    <div className="header-left">
                        <h1 className="page-title">
                            ...
                            {activeNav === "gigs" && <><FiFileText /> Gig Management</>}
                        ...
                        <p className="page-subtitle">
                            ...
                            {activeNav === "gigs" && "Browse and manage platform gigs"}
                ...
                <div className="stats-grid">
                    <div className="stat-card stat-total"><div className="stat-icon"><FiUsers /></div><div className="stat-content"><h2 className="stat-number">{stats.totalUsers}</h2><p className="stat-label">Total Users</p></div><div className="stat-bg-icon"><FiUsers /></div></div>
                    <div className="stat-card stat-sellers"><div className="stat-icon"><FiShoppingBag /></div><div className="stat-content"><h2 className="stat-number">{stats.totalSellers}</h2><p className="stat-label">Freelancers</p></div><div className="stat-bg-icon"><FiShoppingBag /></div></div>
                    <div className="stat-card stat-buyers"><div className="stat-icon"><FaUserGraduate /></div><div className="stat-content"><h2 className="stat-number">{stats.totalBuyers}</h2><p className="stat-label">Students</p></div><div className="stat-bg-icon"><FaUserGraduate /></div></div>
                    <div className="stat-card stat-rate"><div className="stat-icon"><FiFileText /></div><div className="stat-content"><h2 className="stat-number">{stats.totalGigs || 0}</h2><p className="stat-label">Total Gigs</p></div><div className="stat-bg-icon"><FiFileText /></div></div>
                </div>
                ...
                {activeNav === "users" && (
                    <div className="table-section">
                        <div className="table-toolbar">
                            <div className="search-box"><span className="search-icon"><FiSearch /></span><input type="text" placeholder="Search by username or email…" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /></div>
                            <div className="filter-pills">
                                {["all", "faculty", "student"].map((f) => (
                                    <button key={f} className={`pill ${roleFilter === f ? "active" : ""}`} onClick={() => setRoleFilter(f)}>
                                        {f === "all" ? "All Users" : f === "faculty" ? <>🎓 Faculty</> : <><FaUserGraduate /> Students</>}
                                    </button>
                                ))}
                            </div>
                        </div>
                        {loading ? <div className="empty-state"><div className="spinner" /><p>Loading users…</p></div>
                            : error ? <div className="empty-state error-state"><p>{error}</p><button onClick={fetchUsers} className="retry-btn">Retry</button></div>
                                : users.length === 0 ? <div className="empty-state"><p>No users found.</p></div>
                                    : (
                                        <div className="table-wrapper">
                                            <table className="user-table">
                                                <thead><tr><th>User</th><th>Email</th><th>Country</th><th>Role</th><th>Admin</th><th>Joined</th><th>Actions</th></tr></thead>
                                                <tbody>
                                                    {users.map((u) => (
                                                        <tr key={u._id}>
                                                            ...
                                                            <td>{u.country || "—"}</td>
                                                            <td>{creatorRoleBadge(u)}</td>
                                                            <td>{u.isAdmin ? <span className="badge badge-admin"><FiZap className="icon-sm" /> Admin</span> : <span className="badge badge-user">User</span>}</td>
                                                            <td className="date-cell">{new Date(u.createdAt).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" })}</td>
                                                            <td><div className="action-btns"><button className="action-btn edit-btn" title="Edit" onClick={() => setEditUser({ ...u })}><FiEdit2 /></button><button className="action-btn delete-btn" title="Delete" onClick={() => setDeleteTarget(u)} disabled={u._id === currentUser?._id}><FiTrash2 /></button></div></td>
                                                        </tr>
                                                    ))}
                                                </tbody>
                                            </table>
                ...
                {activeNav === "overview" && (
                    <div className="overview-section">
                        ...
                        <div className="chart-row">
                            ...
                            <div className="chart-card">
                                ...
                                <div className="chart-sub-row">
                                    <span className="chart-sub" style={{ color: "#f59e0b" }}>🎓 Faculty: <strong>{stats.totalSellers}</strong></span>
                                    <span className="chart-sub" style={{ color: "#7c3aed" }}>👤 Students: <strong>{stats.totalBuyers}</strong></span>
                                </div>
                            </div>
                            ...
                            <div className="chart-card">
                                ...
                                <div className="chart-sub-row">
                                    <span className="chart-sub" style={{ color: "#46a91f" }}>✓ Completed: <strong>{orderStats.completedOrders}</strong></span>
                                    <span className="chart-sub" style={{ color: "#f59e0b" }}>⏳ Pending: <strong>{orderStats.pendingOrders}</strong></span>
                                </div>
                            </div>
                            ...
                            <div className="chart-card overview-actions-card">
                                ...
                                <div className="quick-actions-vertical">
                                    ...
                                    <button className="quick-btn" onClick={() => setActiveNav("gigs")}><FiFileText /> View Gigs</button>
                                    <button className="quick-btn" onClick={() => setActiveNav("orders")}><FiPackage /> View Orders</button>
                                    {/* <Link to="/" className="quick-btn quick-btn-link"><FiGlobe /> Visit Site</Link> */}
                                </div>
                        ...
                        {/* Row 2: Gigs per category bar */}
                        <div className="chart-card chart-card-full">
                            <h3 className="chart-title"><FiTag /> Gigs per Category</h3>
                            <ResponsiveContainer width="100%" height={240}>
                                <BarChart data={gigsPerCategory} margin={{ top: 8, right: 20, left: -10, bottom: 0 }}>
                                    ...
                                    <Tooltip
                                        ...
                                        contentStyle={{ borderRadius: 10, border: "1px solid #eef2ee", fontSize: 13 }}
                                    />
                                    <Bar dataKey="gigs" radius={[6, 6, 0, 0]} fill="#46a91f" />
                                </BarChart>
                            </ResponsiveContainer>
                        </div>

                        {/* Row 3: Top gigs by sales + top rated */}
                        <div className="chart-row">
                            <div className="chart-card chart-card-half">
                                <h3 className="chart-title"><FiShoppingBag /> {hasSalesData ? "Top Gigs by Sales" : "Top Gigs by Reviews"}</h3>
                                {!hasSalesData && !hasReviewData ? (
                                    <p className="chart-empty">No activity data yet.</p>
                                ) : (
                                    <ResponsiveContainer width="100%" height={220}>
                                        <BarChart
                                            data={hasSalesData ? topGigs : topByReviews}
                                            layout="vertical"
                                        ...
                                        >
                                            ...
                                            <Tooltip contentStyle={{ borderRadius: 10, border: "1px solid #eef2ee", fontSize: 13 }} />
                                            <Bar dataKey={hasSalesData ? "sales" : "reviews"} radius={[0, 6, 6, 0]} fill="#2563eb" />
                                        </BarChart>
                                    </ResponsiveContainer>
                                )}
                            </div>
                            ...
                            <div className="chart-card chart-card-half">
                                <h3 className="chart-title">⭐ Top Rated Gigs</h3>
                                {topRated.length === 0 ? (
                                    ...
                                    <ResponsiveContainer width="100%" height={220}>
                                        <BarChart data={topRated} layout="vertical" margin={{ top: 4, right: 20, left: 10, bottom: 0 }}>
                                            <CartesianGrid strokeDasharray="3 3" stroke="#f0f4f0" horizontal={false} />
                                            <XAxis type="number" domain={[0, 5]} ticks={[1, 2, 3, 4, 5]} tick={{ fontSize: 11, fill: "#888" }} />
                                            <YAxis type="category" dataKey="name" width={110} tick={{ fontSize: 11, fill: "#555" }} />
                                            <Tooltip formatter={(v) => [v + " ★", "Avg Rating"]} contentStyle={{ borderRadius: 10, border: "1px solid #eef2ee", fontSize: 13 }} />
                                            <Bar dataKey="rating" radius={[0, 6, 6, 0]} fill="#f59e0b" />
                                        </BarChart>
                                    </ResponsiveContainer>
                                )}
                            </div>
                ...
                {activeNav === "gigs" && (
                    <div className="table-section">
                        <div className="table-toolbar">
                            <div className="search-box"><span className="search-icon"><FiSearch /></span><input type="text" placeholder="Search by gig title or username…" value={gigSearch} onChange={(e) => setGigSearch(e.target.value)} /></div>

                            {/* ── Category dropdown instead of pills ── */}
                            <div className="cat-dropdown-wrapper" ref={categoryDropdownRef}>
                                ...
                                {catDropdownOpen && (
                                    <div className="cat-dropdown-menu">
                                        <div
                                            className={`cat-dropdown-item ${gigCategoryFilter === "all" ? "active" : ""}`}
                                            onClick={() => { setGigCategoryFilter("all"); setCatDropdownOpen(false); }}
                                        >All Categories</div>
                                        {CATEGORIES.map((cat) => (
                                            <div
                                                key={cat}
                                                className={`cat-dropdown-item ${gigCategoryFilter === cat ? "active" : ""}`}
                                                onClick={() => { setGigCategoryFilter(cat); setCatDropdownOpen(false); }}
                                            >{cat}</div>
                                        ))}
                                    </div>
                                )}
                        ...
                        {gigsLoading ? <div className="empty-state"><div className="spinner" /><p>Loading gigs…</p></div>
                            : gigsError ? <div className="empty-state error-state"><p>{gigsError}</p><button onClick={fetchGigs} className="retry-btn">Retry</button></div>
                                : gigs.length === 0 ? <div className="empty-state"><div className="empty-icon"><FiInbox /></div><p>No gigs found.</p></div>
                                    : (
                                        <div className="table-wrapper">
                                            <table className="user-table">
                                                <thead><tr><th>Gig</th><th>Category</th><th>Creator</th><th>Role</th><th>Price</th><th>Sales</th><th>Date</th><th>Actions</th></tr></thead>
                                                <tbody>
                                                    {gigs.map((g) => (
                                                        <tr key={g._id}>
                                                            <td>
                                                            ...
                                                            {/* ── Role badge column ── */}
                                                            <td>{creatorRoleBadge(g.userId)}</td>
                                                            ...
                                                            <td>
                                                                <div className="action-btns">
                                                                    <button className="action-btn view-btn" title="View Gig" onClick={() => setViewGigTarget(g)}><FiEye /></button>
                                                                    <button className="action-btn delete-btn" title="Delete Gig" onClick={() => setDeleteGigTarget(g)}><FiTrash2 /></button>
                                                                </div>
                                                            </td>
                                                        </tr>
                                                    ))}
                        ...
                        <div className="table-footer">Showing <strong>{gigs.length}</strong> gig{gigs.length !== 1 && "s"}</div>
                    </div>
                )}
                ...
                {activeNav === "orders" && (
                    <>
                        ...
                        <div className="table-section">
                            <div className="table-toolbar">
                                <div className="search-box"><span className="search-icon"><FiSearch /></span><input type="text" placeholder="Search by buyer, seller, gig or order ID…" value={orderSearch} onChange={(e) => setOrderSearch(e.target.value)} /></div>
                                <div className="filter-pills">
                                        ...
                                        : (
                                            <div className="table-wrapper">
                                                <table className="user-table orders-table">
                                                    <thead><tr><th>Order ID</th><th>Gig</th><th>Buyer</th><th>Seller</th><th>Price</th><th>Status</th><th>Date</th><th>Actions</th></tr></thead>
                                                    <tbody>
                                                        {filteredOrders.map((o) => (
                                                            <tr key={o._id}>
                                                                ...
                                                                <td><div className="action-btns"><button className="action-btn view-btn" title="View Details" onClick={() => setSelectedOrder(o)}><FiEye /></button><button className={`action-btn ${o.isCompleted ? "undo-btn" : "complete-btn"}`} title={o.isCompleted ? "Mark Pending" : "Mark Completed"} onClick={() => handleUpdateOrderStatus(o._id, !o.isCompleted)}>{o.isCompleted ? <FiCornerUpLeft /> : <FiCheck />}</button></div></td>
                                                            </tr>
                                                        ))}
                                                    </tbody>
                                                </table>
                            ...
                            <div className="table-footer">Showing <strong>{filteredOrders.length}</strong> order{filteredOrders.length !== 1 && "s"}{orderStats.totalRevenue > 0 && <span className="footer-revenue"> · Revenue from completed: <strong>{formatCurrency(orderStats.totalRevenue)}</strong></span>}</div>
                        </div>
                    </>
                )}
            </main>
                    ...
                    <div className="modal" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="modal-body">
                            ...
                            <div className="form-grid">
                                ...
                                <div className="form-group checkbox-group"><label className="checkbox-label"><input type="checkbox" checked={editUser.isSeller} onChange={(e) => setEditUser({ ...editUser, isSeller: e.target.checked })} /><span>Is Seller</span></label></div>
                ...
                <div className="modal-overlay" onClick={() => setViewGigTarget(null)}>
                    <div className="modal modal-gig-edit" onClick={(e) => e.stopPropagation()}>
                        <div className="modal-header">
                            <h2><FiEye /> Gig Details</h2>
                            <button className="modal-close" onClick={() => setViewGigTarget(null)}><FiX /></button>
                        </div>
                        <div className="modal-body">
                            {/* Cover + meta */}
                            <div className="gig-edit-preview">
                                ...
                                <div className="gig-edit-meta">
                                    ...
                                    <p className="gig-edit-creator">
                                        By <strong>{viewGigTarget.userId?.username || "—"}</strong>
                                        <span className="gig-creator-role">{creatorRoleBadge(viewGigTarget.userId)}</span>
                                    </p>
                            ...
                            <div className="gig-view-grid">
                                <div className="gig-view-item full">
                                    <span className="gig-view-label">Gig Title</span>
                                    <span className="gig-view-value large">{viewGigTarget.title || "—"}</span>
                                </div>
                                <div className="gig-view-item full">
                                    <span className="gig-view-label">Service Title</span>
                                    <span className="gig-view-value">{viewGigTarget.shortTitle || "—"}</span>
                                </div>
                                <div className="gig-view-item">
                                    <span className="gig-view-label">Category</span>
                                    <span className="badge badge-cat">{viewGigTarget.cat || "—"}</span>
                                </div>
                                <div className="gig-view-item">
                                    <span className="gig-view-label">Price</span>
                                    <span className="gig-view-value green">{formatCurrency(viewGigTarget.price)}</span>
                                </div>
                                <div className="gig-view-item">
                                    <span className="gig-view-label">Delivery Time</span>
                                    <span className="gig-view-value">{viewGigTarget.deliveryTime || "—"} day{viewGigTarget.deliveryTime !== 1 ? "s" : ""}</span>
                                </div>
                                <div className="gig-view-item">
                                    <span className="gig-view-label">Revisions</span>
                                    <span className="gig-view-value">{viewGigTarget.revisionNumber ?? "—"}</span>
                                </div>
                                <div className="gig-view-item">
                                    <span className="gig-view-label">Sales</span>
                                    <span className="gig-view-value">{viewGigTarget.sales || 0}</span>
                                </div>
                                <div className="gig-view-item full">
                                    <span className="gig-view-label">Short Description</span>
                                    <span className="gig-view-value">{viewGigTarget.shortDesc || "—"}</span>
                                </div>
                                <div className="gig-view-item full">
                                    <span className="gig-view-label">Full Description</span>
                                    <p className="gig-view-desc">{viewGigTarget.desc || "—"}</p>
                                </div>
                                {viewGigTarget.features?.length > 0 && (
                                    <div className="gig-view-item full">
                                        <span className="gig-view-label">Features</span>
                                        <div className="gig-features-list">
                                            {viewGigTarget.features.map((f) => <span key={f} className="gig-feature-tag">{f}</span>)}
                                        </div>
                                    </div>
                                )}
                                {viewGigTarget.images?.length > 0 && (
                                    <div className="gig-view-item full">
                                        <span className="gig-view-label">Additional Images</span>
                                        <div className="gig-view-images">
                                            {viewGigTarget.images.map((img, i) => (
                                                <img key={i} src={img} alt="" className="gig-view-thumb" />
                                            ))}
                                        </div>
                                    </div>
                                )}
                        ...
                        <div className="modal-footer">
                            ...
                            <button className="btn-danger" onClick={() => { setDeleteGigTarget(viewGigTarget); setViewGigTarget(null); }}>
                                <FiTrash2 /> Delete Gig
                            </button>
                        </div>
                    </div>
            ...
            {deleteGigTarget && (
                <div className="modal-overlay" onClick={() => setDeleteGigTarget(null)}>
                    <div className="modal modal-sm" onClick={(e) => e.stopPropagation()}>
                        <div className="modal-header danger"><h2><FiTrash2 /> Delete Gig</h2><button className="modal-close" onClick={() => setDeleteGigTarget(null)}><FiX /></button></div>
                        ...
                        <div className="modal-footer"><button className="btn-secondary" onClick={() => setDeleteGigTarget(null)}>Cancel</button><button className="btn-danger" onClick={handleDeleteGig} disabled={deleting}>{deleting ? "Deleting…" : "Delete Gig"}</button></div>
                    </div>
                </div>
            )}
                    ...
                    <div className="modal modal-order" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="modal-body">
                            ...
                            <div className="order-detail-section">
                                <h4 className="detail-section-title"><FiFileText /> Gig</h4>
